@php
    ...
    // ----------------------------------------------------------------------
    $change = $change ?? false;
    $type = 'select'; // Select type does not have custom styling
    $size = $size ?? false; // Default size is medium
    // ----------------------------------------------------------------------

    // ----------------------------------------------------------------------
    if( $model ){
        if( is_string( $model )) $model = (object) array( 'type' => 'default', 'name' => $model );
        elseif( is_array( $model ) && isset( $model[0], $model[1] )){
            $model = (object) array( 'type' => $model[0], 'name' => $model[1] );
    ...
    <div class="{{ $classGroup->join(' ') }}" v-for="name in [{!! $name !!}]" :class="{!! $disabled !!} ? 'input-static': '{{ "input-{$type}" }}'">
        ...
        <template>
            <select :name="name" :id="name" class="{{ $classList->join(' ')}}"
                @if( $model && !empty( $model->name ) && !empty( $model->type ))
                    @if( 'default' == $model->type ) v-model="{{ $model->name }}" @endif
                    @if( 'number' == $model->type ) v-model.number="{{ $model->name }}" @endif
                @endif
                :class="{!! $disabled !!} ? 'input-static': '{{ "input-{$type}" }}'"
                @if( $placeholder ) :placeholder="{!! $placeholder !!}" @endif
                @if( !empty( $change )) @change="{{ $change }}" @endif @if( $disabled ) :disabled="{{ $disabled }}" @endif
