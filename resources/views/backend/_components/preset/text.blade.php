@php
    ...
    // ----------------------------------------------------------------------
    $type = 'text'; // Text type does not have custom styling
    $maxlength = $maxlength ?? 128; // Default text maxlength
    ...
    <div class="{{ $classGroup->join(' ') }}" v-for="name in [{!! $name !!}]" :class="{!! $disabled !!} ? 'input-static': '{{ "input-{$type}" }}'">
        ...
        <template>
            <input type="text" class="{{ $classList->join(' ')}}" :name="name" :id="name" 
                @if( $model ) v-model="{{ $model }}" @endif @if( $disabled ) :disabled="{{ $disabled }}" @endif
                @if( $validation ) data-parsley-maxlength="{{ $maxlength }}" data-parsley-trigger="change focusout" data-parsley-no-focus @endif 
